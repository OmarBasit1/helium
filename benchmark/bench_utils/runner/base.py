...
class LLMServerConfig:
    ...
    def extend_context_length(
        ...
        # vLLM v0.16.0 reads rope_parameters (not rope_scaling) for both
        # max-length validation and RoPE computation. rope_scaling is patched
        # into rope_parameters by patch_rope_parameters() during get_config(),
        # which runs before hf_overrides dict-values are applied, so setting
        # rope_scaling here would have no effect on the length check.
        hf_overrides = {
            "rope_parameters": {
                "rope_type": "yarn",
                "factor": factor,
...
class BenchmarkRunner:
    ...
    async def run(
        ...
        for args, kwargs in tqdm.tqdm(
            ...
            bench.reset_timer()
            await self._run_bench(bench, *args, **kwargs)
